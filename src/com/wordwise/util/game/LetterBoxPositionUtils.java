...
public class LetterBoxPositionUtils
{
	...
	public static boolean InsertTranslationIntoPosition(String word, int positionOfFirstLetter, List<String> letterBox)
	{
		...
		while (directions.size() > 0)
		{
			...
			while (ways.size() > 0)
			{
				...
				if (Insert(word, positionOfFirstLetter, direction, way, letterBox))
				{
					return true;
				}
	...
	private static boolean Insert(String word, int positionOfFirstLetter, Direction direction, Way way, List<String> letterBox)
	{
		if (!IsPossibleToInsert(word, positionOfFirstLetter, direction, way, letterBox))
		{
			return false;
		}
		...
		{
			letterBox.set(GetPosition(point), String.valueOf(word.charAt(i)).toLowerCase());
			point = GetNextPoint(point, direction, way);
		}
		return true;
	}
	
	private static Point GetNextPoint(Point initialPoint, Direction direction, Way way)
	{
	...
	private static boolean IsPossibleToInsert(String word, int positionOfFirstLetter, Direction direction, Way way,	List<String> letterBox)
	{
		Point point = GetPoint(positionOfFirstLetter);
		for (int i = 0; i < word.length(); i++)
		{
	...
	private static boolean IsPossibleToInsert(char letter, Point point, List<String> letterBox)
	{
		if (!IsPointInsideLetterBox(point, letterBox))
		{
			return false;
		}
		if (letterBox.get(GetPosition(point)).charAt(0) == letterBox.get(GetPosition(point)).toLowerCase().charAt(0) &&
	...
	private static boolean IsPointInsideLetterBox(Point point, List<String> letterBox)
	{
		Point limitPoint = GetPoint(letterBox.size()-1);
		if (point.x < 0 || point.x > limitPoint.x ||
			point.y < 0 || point.y > limitPoint.y)
		{
			return false;
		}
		return true;
